...
from WT_TRANS t1
  ...
  on t1.WT_STATUS_FLAG_ID = t2.WT_STATUS_FLAG_ID AND t1.IS_ACTIVE =1
  LEFT JOIN (SELECT t1.WT_TRANS_ID ,t3.AGENCY_FULL_NAME AS SELLERFULL,
  ...
  LEFT JOIN (SELECT t1.WT_TRANS_ID  ,
        ...
        TAB_TO_STRING(CAST(COLLECT(t3.AGENCY_FULL_NAME) AS T_VARCHAR2_TAB)) AS BUYERSFULL
      FROM WT_TRANS t1
        JOIN WT_BUYER t2
          JOIN WT_AGENCY t3
          ON t2.WT_AGENCY_ID = t3.WT_AGENCY_ID
