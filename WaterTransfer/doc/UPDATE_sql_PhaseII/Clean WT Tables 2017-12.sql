...
--------------------------------------------------------------------------------
--  Clean Tables and Reset Sequence
--------------------------------------------------------------------------------
...
--Clean TABLE APP_USER;
select USER_PKSEQ.nextval from dual;
EXEC reset_seq_to('USER_PKSEQ', 70);

--Clean Table WT_TRANS_TRACK
DELETE FROM WT_TRANS_TRACK;
select WT_TRANS_TRACK_PKSEQ.nextval from dual;
EXEC reset_seq('WT_TRANS_TRACK_PKSEQ');

--Clean Table WT_TRANS_FILE
DELETE FROM WT_TRACK_FILE;
select WT_TRACK_FILE_PKSEQ.nextval from dual;
EXEC reset_seq('WT_TRACK_FILE_PKSEQ');

--Clean Table WT_STATUS_TRACK
DELETE FROM WT_STATUS_TRACK;
select WT_STATUS_TRACK_PKSEQ.nextval from dual;
EXEC reset_seq('WT_STATUS_TRACK_PKSEQ');
SELECT WT_TRANS_ID FROM WT_TRANS WHERE WT_STATUS_FLAG_ID=10 AND IS_ACTIVE=1 ORDER BY WT_TRANS_ID;

--Clean Link Tables
DELETE FROM WT_RV_PURPOSE;
DELETE FROM WT_BUYERS_CONTACT;

-- Delete NOTE tables
DELETE FROM WT_INTERNAL_NOTE;
EXEC reset_seq('WT_INTERNAL_NOTE_PKSEQ');
DELETE FROM WT_REVIEW_NOTE;
EXEC reset_seq('WT_REVIEW_NOTE_PKSEQ');

--Clean Table WT_PRE_TRASFER
DELETE FROM WT_PRE_TRANSFER;
select WT_PRE_TRANSFER_PKSEQ.nextval from dual;
EXEC reset_seq('WT_PRE_TRANSFER_PKSEQ');

--Clean Table WT_CI_CROPTYPE
DELETE FROM WT_CI_CROPTYPE;
select WT_CI_CROPTYPE_PKSEQ.nextval from dual;
EXEC reset_seq('WT_CI_CROPTYPE_PKSEQ');

--Clean Attachment link Tables
DELETE FROM WT_CI_ATTACHMENT;

--Update Status of History Proposals
SELECT * FROM WT_TRANS WHERE WT_STATUS_FLAG_ID = 6;
UPDATE WT_TRANS SET WT_STATUS_FLAG_ID = 10 WHERE WT_STATUS_FLAG_ID = 6;
SELECT * FROM WT_TRANS WHERE WT_STATUS_FLAG_ID = 10;
SELECT * FROM WT_TRANS WHERE CREATED_BY_ID = 263;
SELECT WT_TRANS_ID, WT_TRANS_NUM, IS_ACTIVE FROM WT_TRANS WHERE CREATED_BY_ID = 263;
UPDATE WT_TRANS SET IS_ACTIVE = 1 WHERE CREATED_BY_ID = 263;
--SELECT * FROM WT_TRANS WHERE CREATED_BY_ID = 361;
--SELECT * FROM WT_TRANS WHERE UPDATED_BY_ID = 361;
--SELECT * FROM WT_TRANS WHERE CREATED_BY_ID = 644;
--SELECT * FROM WT_TRANS WHERE CREATED_BY_ID = 401;
--SELECT * FROM WT_TRANS WHERE CREATED_BY_ID = 506;
...
--EXEC reset_seq_to('WT_AGENCY_PKSEQ', 91);
...
SELECT * FROM WT_BUYER;
DELETE FROM WT_BUYER;
EXEC reset_seq('WT_BUYER_PKSEQ');
TRUNCATE TABLE WT_BUYERS_CONTACT;

SELECT * FROM WT_SELLER;
DELETE FROM WT_SELLER;
EXEC reset_seq('WT_SELLER_PKSEQ');
...
DELETE FROM WT_TRANS_ATTACHMENTS WHERE WT_ATTACHMENT_ID > 0;
DELETE FROM WT_GW_ATTACHMENT WHERE WT_ATTACHMENT_ID > 0;
DELETE FROM WT_CI_ATTACHMENT WHERE WT_ATTACHMENT_ID > 0;
DELETE FROM WT_CI_MAP_ATTACHMENT WHERE WT_ATTACHMENT_ID > 0;
DELETE FROM WT_RV_ATTACHMENT WHERE WT_ATTACHMENT_ID > 0;
DELETE FROM WT_WELL_ATTACHMENT WHERE WT_ATTACHMENT_ID > 0;

DELETE FROM WT_ATTACHMENT WHERE WT_ATTACHMENT_ID > 0;
--EXEC reset_seq_to('WT_ATTACHMENT_PKSEQ', 345);
EXEC reset_seq('WT_ATTACHMENT_PKSEQ');
...
DELETE FROM WT_TRANS;
EXEC reset_seq('WT_TRANS_PKSEQ');
--------------------------------------------------------------------------------
--  Drop Unused Tables
--------------------------------------------------------------------------------
