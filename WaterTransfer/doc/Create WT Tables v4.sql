...
--------------------------------------------------------------------------------
--  DDL for Table WT_SELLER_BUYER
--------------------------------------------------------------------------------
CREATE TABLE WT_SELLER_BUYER (
  WT_SELLER_BUYER_ID NUMERIC(16,0) PRIMARY KEY
  ,WT_TRANS_ID REFERENCES WT_TRANS(WT_TRANS_ID)
  ,WT_AGENCY_ID REFERENCES WT_AGENCY(WT_AGENCY_ID)
  ,SELLER_OR_BUYER VARCHAR2(1) DEFAULT 'S' NOT NULL
);
/

CREATE SEQUENCE WT_SELLER_BUYER_PKSEQ  
  START WITH 1
  INCREMENT BY 1;
/

CREATE OR REPLACE TRIGGER WT_SELLER_BUYER_PKTRIG
BEFORE INSERT
ON WT_SELLER_BUYER
FOR EACH ROW
BEGIN
  IF :NEW.WT_SELLER_BUYER_ID IS NULL THEN
    SELECT WT_SELLER_BUYER_PKSEQ.NEXTVAL
    INTO :NEW.WT_SELLER_BUYER_ID
    FROM DUAL;
  END IF;
END WT_SELLER_BUYER;
/
